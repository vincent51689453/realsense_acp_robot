...
def main():
    ...
    # Marker Publisher
    hc.marker_vis = rospy.Publisher(hc.marker_topic,Marker,queue_size=10)

    # TF Subscriber
    hc.tf_listener = tf.TransformListener()
    ...
    while True:
        if((hc.depth_image is not None)and(hc.color_image is not None)):
            # Processing Core
            color_output, depth_output = alogrithm.core(hc.color_image,hc.depth_image,hc.intrinsic)
